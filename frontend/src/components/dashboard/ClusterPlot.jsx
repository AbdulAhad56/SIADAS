...
export default function ClusterPlot({ clustering }) {
  ...
  // Subsample for perf
  const points  = (cluster_points || []).length > 600
    ? (cluster_points || []).filter((_, i) => i % Math.ceil(cluster_points.length / 600) === 0)
    : (cluster_points || []);
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full
          ...
          <span className="font-semibold text-primary">Optimal K = {optimal_k}</span>
        </div>
      </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
              ...
              {series.map(({ name, color, data }) => (
                <Scatter key={name} name={name} data={data}
                  fill={color} fillOpacity={0.75} r={3} />
              ))}
            </ScatterChart>
          </ResponsiveContainer>
        </div>
      ...
      {elbowData.length > 0 && (
        <div>
          ...
          <div style={{ width: "100%", height: "176px" }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={elbowData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
                <XAxis dataKey="k" label={{ value: "K", position: "insideRight", offset: 0,
                  fontSize: 10, fill: "var(--color-text-muted)" }}
                  tick={{ fontSize: 10, fill: "var(--color-text-muted)" }} />
                <YAxis tick={{ fontSize: 9, fill: "var(--color-text-muted)" }} width={50}
                  tickFormatter={(v) => v > 999 ? `${(v/1000).toFixed(0)}k` : v} />
                <Tooltip content={<ElbowTip />} />
                <ReferenceLine x={optimal_k} stroke="#EF4444"
                  strokeDasharray="4 3" label={{ value: `K=${optimal_k}`,
                  fontSize: 10, fill: "var(--color-danger)", position: "top" }} />
                <Line type="monotone" dataKey="inertia" name="Inertia"
          ...
          <p className="text-[10px] text-text-muted mt-1">
            Red line marks the elbow point (optimal K={optimal_k}) detected via perpendicular-distance heuristic.
          </p>
        </div>
      )}
    </div>
