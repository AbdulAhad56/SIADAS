...
function confidenceColor(v) {
  ...
  if (v >= 0.7) return { bg: "rgba(79,70,229,0.10)", color: "#4F46E5" };
  ...
  return (
    <span
      className="inline-block px-2 py-0.5 rounded-md text-[11px] font-medium
                     ...
                     text-text-secondary mr-1 mb-0.5 whitespace-nowrap"
    >
      {text}
    </span>
  );
...
export default function AssociationRules({ data }) {
  ...
  if (!rules.length) {
    return (
      <EmptyState msg="No association rules found at the current confidence threshold (≥ 0.5)." />
    );
  }
  ...
  return (
    <div className="space-y-5">
                  ...
                  >
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <div
                          className="w-12 h-1.5 bg-surface-border
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span
                        className="px-2 py-0.5 rounded-full text-xs font-bold"
                        style={{
                          background: liftStyle.bg,
                          color: liftStyle.color,
                        }}
                      >
                        {rule.lift.toFixed(2)}×
                      </span>
      ...
      {/* ── Legend ──────────────────────────────────────────────────── */}
      <div
        className="flex flex-wrap gap-4 text-[11px] text-text-muted
                      pt-1 border-t border-surface-border"
      >
        <span>
        ...
        <span>
          <strong className="text-text-secondary">Lift</strong> —
